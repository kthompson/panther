...
class AssemblyParser
{
    ...
    public AssemblerListing ParseListing()
    {
        ...
        return new AssemblerListing(_sourceFile, instructions.ToImmutable());
    }
    ...
    private Instruction ParseFunctionInstruction()
    {
        ...
        return new FunctionInstruction(_sourceFile, code, label, operand);
    }
    ...
    private Instruction ParseCallInstruction()
    {
        ...
        var operand = Accept(SyntaxKind.NumberToken);
        AssertStatementTerminator(operand);
        return new CallInstruction(_sourceFile, code, label, operand);
    }

    private Instruction ParseLoadStringInstruction()
    {
        var code = AcceptOpCode();
        var operand = Accept(SyntaxKind.StringToken);
        AssertStatementTerminator(operand);

        return new StringOperandInstruction(_sourceFile, code, operand);
    }

    private Instruction ParseLabelOperandInstruction()
    {
        var code = AcceptOpCode();
        var label = Accept(SyntaxKind.IdentifierToken);
        AssertStatementTerminator(label);

        return new LabelOperandInstruction(_sourceFile, code, label);
    }

    private Instruction ParseIntOperandInstruction()
    {
        var code = AcceptOpCode();
        var number = Accept(SyntaxKind.NumberToken);
        AssertStatementTerminator(number);

        return new IntOperandInstruction(_sourceFile, code, number);
    }

    private Instruction ParseNoOperandInstruction()
    {
        var code = AcceptOpCode();
        AssertStatementTerminator(code);

        return new NoOperandInstruction(_sourceFile, code);
    }
